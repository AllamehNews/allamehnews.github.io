var news_data = {
  "news-1": {
    title: "شروع کار علامه نیوز",
    description:
      "با شروع روند انتخابات شورای نهم، علامه نیوز کار خود را شروع کرد.",
    page: "./articles/news-1",
    image: "./assets/News/news-1.jpg",
    date: "16 مهر 1401",
    isHot: false,
    isRecommended: true,
    isSponsored: false,
  },
  "news-2": {
    title: "ترافیک بچه های حلی 3 را کلافه کرد",
    description:
      "هنوز مشخص نیست که آیا دوباره برنامه تغییر میکند و ساعت 1:30 تعطیل میشویم یا خیر",
    page: "none",
    image: "./assets/News/news-2.jpg",
    date: "16 مهر 1401",
    isHot: false,
    isRecommended: true,
    isSponsored: false,
  },
  "news-4": {
    title: "استرس بعضی از بچه ها در مورد امتحان و مشق برنامه نویسی",
    description:
      "بعضی از بچه های پایه نهم که از فناوری پارسال HTML را به خوبی یاد نگرفته اند ترس شدیدی برای آزمون برنامه نویسی دارند، ولی ما اینجاییم تا به شما منابع آموزشی جذابی بدیم، البته اگر نت اجازه دهد!",
    page: "./articles/news-4",
    image: "./assets/News/news-4.jpg",
    date: "16 مهر 1401",
    isHot: false,
    isRecommended: false,
    isSponsored: false,
  },
  "news-3": {
    title: "بچه ها در انتظار انتخابات شورا",
    description:
      "بچه ها بی صبرانه منتظر رقابتی که در انتخابات شورای نهم اتفاق خواهد افتاد هستند.",
    page: "./articles/news-3",
    image: "./assets/News/news-3.png",
    date: "16 مهر 1401",
    isHot: false,
    isRecommended: false,
    isSponsored: false,
  },
  "news-5": {
    title: "بچه ها در انتظار ترافیک چهارشنبه",
    description:
      "بر اساس اعلامیه های فراوان بنظر میرسید که فردا چهارشنبه ترافیک عظیمی در تهران اتفاق خواهد افتاد و خیلی از بچه ها بابت امنیت خود در روز چهارشنبه نگران هستند",
    page: "none",
    image: "./assets/News/news-2.jpg",
    date: "19 مهر 1401",
    isHot: false,
    isRecommended: false,
    isSponsored: false,
  },
  "news-6": {
    title: "تست هوش هیجانی غیر استاندارد",
    description:
      "تست هوش هیجانی دفتر پایه نهم (که در اصل تست mbti است) بجای استفاده از تست های استاندارد 5 گزینه ای همچون 16personalities از تست دو گزینه ای karboom استفاده کرده است و این موضوع باعث اشتباه در آمدن نتیجه ی برخی از دانش آموزان شده است",
    page: "none",
    image: "./assets/News/news-6.png",
    date: "19 مهر 1401",
    isHot: true,
    isRecommended: false,
    isSponsored: false,
  },
  "news-7": {
    title: "بچه ها در انتظار شب مانی در مدرسه",
    description:
      "پس از برگزار نشدن شب مانی در پایه هشتم، بالاخره بعد از کلی میم و بازی ترسناک شب مانی پایه نهم برگزار خواهد شد. برای اطلاعات بیشتر کلیک کنید",
    page: "./articles/news-5",
    image: "./assets/News/news-7.jpg",
    date: "26 مهر 1401",
    isHot: false,
    isRecommended: true,
    isSponsored: false,
  },
  "news-8": {
    title: "اولین آزمون جامع پایه نهم با چاشنی بیماری",
    description:
      "اولین آزمون جامع پایه نهم که شامل دروس شیمی، زیست، فیزیک، ریاضی، تاریخ، معارف و عربی می باشد در دوشنبه در حین کلاس مشاوره برگزار خواهد شد. حتی با اینکه این هفته حدودا نصف مدرسه مریض بوده اند و توانایی خواندن درس را نداشته و هنوز ندارند.",
    page: "none",
    image: "./assets/News/news-8.png",
    date: "29 مهر 1401",
    isHot: false,
    isRecommended: false,
    isSponsored: false,
  },
  "news-9": {
    title: "کنسل شدن انتخابات شورای پایه نهم به دلیل کمبود کاندید ها",
    description:
      "بنا بر اطلاعاتی که به دست ما رسیده است به دلیل کم بودن کاندید های شورا، انتخاباتی برگزار نخواهد شد و کاندید ها به شکل مستقیم وارد شورا خواهند شد.",
    page: "./articles/news-9",
    image: "./assets/News/news-9.jpg",
    date: "29 مهر 1401",
    isHot: true,
    isRecommended: true,
    isSponsored: false,
  },
  "news-10": {
    title: "نسل شدن آزمون جامع دوشنبه به دلیل شیوع بیماری",
    description:
      "بنا بر اعلام آشیانه 12 به دلیل شیوع بیماری در مدرسه آزمون جامع دوشنبه پایه نهم کنسل شد و در زمان دیگری برگزار خواهد شد",
    page: "none",
    image: "./assets/News/news-8.png",
    date: "1 آبان 1401",
    isHot: true,
    isRecommended: true,
    isSponsored: false,
  },
  "news-11": {
    title: "استفاده بخش هایی از لوگوی ائتلاف تکنو در لوگوی ائتلاف طمساح",
    description:
      "چندی پیش در لوگوی منتشر شده از ائتلاف طمساح، مشاهده شد که بخش هایی از لوگوی ائتلاف تکنو (سه مداد / قلم بالای لوگو) مستقیما کپی شده است. این موضوع توسط کارشناسان ما ثابت شده است به دلیل اینکه کد رنگی قلم های هر دو لوگو یکسان هستند.",
    page: "none",
    image: "./assets/News/news-11.png",
    date: "1 آبان 1401",
    isHot: true,
    isRecommended: true,
    isSponsored: false,
  },
};
